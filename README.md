...
Search every NASDAQ, NYSE, BSE and NSE listing, see backtested price forecasts, and screen exchanges by
volume, buying pressure, profitability and momentum. OAuth login (Google / GitHub), dark and light mode.
...
The model (ridge regression + gradient-boosted trees on 23 technical features) is evaluated with a
walk-forward backtest. It picks its model on one year and is scored on the following, untouched year. Every
metric is shown next to a baseline:

- **Price accuracy** (100% − mean absolute % error) is 93–99% for almost any stock, but predicting "no change"
  scores about the same. The page shows both numbers side by side.
- **Direction right** is typically 45–65%. The baseline is always guessing the more common direction.

If no model beat "no change" in validation, the page shows a flat forecast and says "No edge found".
